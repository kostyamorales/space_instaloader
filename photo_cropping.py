...
def resize_images(path_name_pictures):
    for path_name_picture in path_name_pictures:
        ...
        try:
            ...
            if image.width / image.height > 1.91:
                image = cut_width_image(image)
            if image.width / image.height < 0.8:
                image = cut_height_image(image)
            if image.mode != 'RGB':
...
def delete_file(picture_path):
    original_file = Path(picture_path)
    original_file.unlink()


def cut_width_image(image):
